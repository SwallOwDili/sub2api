...
// isOpenAICodexGatewayOwnedIdentityHeader reports headers whose upstream value
// describes the HTTP client talking to ChatGPT Codex. For OAuth Codex accounts
// that client is this gateway, not the downstream caller (including callers
// entering through Anthropic/Chat Completions protocol conversion). These values
// are generated by the gateway while identity enforcement is enabled. Explicitly
// disabling enforcement restores the legacy whitelist + final UA pairing path.
func isOpenAICodexGatewayOwnedIdentityHeader(lowerKey string) bool {
...
func shouldCopyOpenAIInboundHeader(account *Account, lowerKey string) bool {
	return account == nil || !account.UsesOpenAICodexProtocol() ||
		!isOpenAICodexGatewayOwnedIdentityHeader(lowerKey) || !codexIdentityEnforcement.Load()
}
